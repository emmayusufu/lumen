...
interface Props {
  ...
  onUpdateRole: (docId: string, role: DocCollaboratorRole) => Promise<void>;
}
...
function roleSummary(roles: string[]): string {
  ...
  if (counts.editor)
    parts.push(
      `editor on ${counts.editor} doc${counts.editor === 1 ? "" : "s"}`,
    );
  if (counts.viewer)
    parts.push(
      `viewer on ${counts.viewer} doc${counts.viewer === 1 ? "" : "s"}`,
    );
  return parts.join(" · ");
}
    ...
    >
      ...
      >
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Typography
            sx={{ fontSize: "0.74rem", color: "text.disabled", mt: 0.25 }}
          >
      ...
      {open && (
        ...
        >
          ...
          {person.docs.map((d, i) => (
            ...
            >
              <Typography
                sx={{ fontSize: "0.84rem", flex: 1, minWidth: 0 }}
                noWrap
              >
                {d.doc_title}
              </Typography>
              <Select
                ...
                onChange={(e) => {
                  ...
                  void onUpdateRole(
                    d.doc_id,
                    e.target.value as DocCollaboratorRole,
                  );
                }}
            ...
            {!confirmBulk ? (
              ...
              >
                <Typography sx={{ fontSize: "0.82rem" }}>
                  Remove {person.display_name || person.email} from all{" "}
                  {person.doc_count} doc
