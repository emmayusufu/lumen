...
def test_signup_creates_user_and_sets_cookie():
    ...
    with patch(
        "app.routers.auth.workspaces_db.create_workspace_and_admin",
        new_callable=AsyncMock,
        return_value=(_uuid.uuid4(), "acme-ab12", "user1"),
    ):
    ...
    assert resp.json()["workspace"]["slug"] == "acme-ab12"
...
def test_login_valid_credentials_sets_cookie():
    ...
    with patch(
        "app.routers.auth.db.get_user_for_auth", new_callable=AsyncMock, return_value=user_data
    ):
        resp = TestClient(app).post(
            "/api/v1/auth/login",
...
def test_me_returns_current_user():
    app.dependency_overrides[real_current_user] = lambda: User(
        id="user1", email="alice@acme.com", name="Alice Smith"
    )
    try:
...
def test_logout_with_cookie_revokes_and_clears():
    ...
    token = create_token("user1", "alice@acme.com", "Alice Smith")
    with (
