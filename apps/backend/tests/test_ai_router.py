...
def test_inline_endpoint_streams_sse_events():
    ...
    try:
        with (
            patch(
                "app.routers.ai.workspaces_db.list_workspaces_for_user",
                new_callable=AsyncMock,
                return_value=_WS_LIST,
            ),
            patch("app.routers.ai.get_user_llm", new_callable=AsyncMock, return_value=_fake_llm()),
        ):
