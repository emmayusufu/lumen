...
async def test_get_user_key_returns_decrypted_value():
    ...
    mock_pool.return_value.fetchrow = AsyncMock(
        return_value={"deepseek_key_enc": enc_value}
    )
    with patch("app.db.credentials.pool", mock_pool):
        result = await creds.get_user_key("user1")
...
@pytest.mark.asyncio
async def test_get_org_key_returns_decrypted_value():
    ...
    with patch("app.db.credentials.pool", mock_pool):
        result = await creds.get_org_key("org1")
    assert result == "sk-org"


@pytest.mark.asyncio
async def test_set_org_key_upserts_encrypted():
    captured = {}
    ...
    mock_pool.return_value.execute = fake_execute
    with patch("app.db.credentials.pool", mock_pool):
        await creds.set_org_key("org1", "sk-org")
    assert "ON CONFLICT" in captured["sql"]
    assert captured["args"][0] == "org1"
